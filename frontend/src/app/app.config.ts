...
export class AppComponent {
  ...
  startLoadingPhases() {
    ...
    // Simulate progress: 0–25% Uploading, 25–60% Analyzing, 60–95% Colorizing
    // Stops at 95% — jumps to 100% when response arrives
    const phases = [
      { until: 25, text: 'Uploading…' },
      { until: 60, text: 'Analyzing…' },
      { until: 95, text: 'Colorizing…' },
    ];
    ...
    this.progressTimer = setInterval(() => {
      ...
      this.loadingProgress = Math.min(this.loadingProgress + Math.random() * 2, 95);
      const p = Math.round(this.loadingProgress);
  ...
  stopLoadingPhases() {
    ...
    this.blobBackground = this.makeBlobBg(100);
